<?php
  ...
  function has_unique_shippername($shippername, $current_id="0") {
    $shipper = Shipper::find_by_shippername($shippername);
    if($shipper === false || $shipper->id == $current_id) {
      // is unique
      return true;
    } else {
      // not unique
    ...
    function has_unique_email($client_email, $current_id="0") {
      ...
      $email = Users::find_by_email($client_email);
  ...
  function has_unique_waybillno($waybillno, $current_id="0") {
    $trans = Transaction::find_by_wayBillNum($waybillno);
